...
const MENU = [
  { name: "Dashboard", icon: LayoutDashboard, path: "/approver/dashboard" },
  { name: "Approvals", icon: ClipboardCheck,  path: "/approver/approvals" },
  { name: "Profile",   icon: User,            path: "/approver/profile"   },
];

const ACTIVE_BG    = "#141617";
const ACTIVE_COLOR = "#fff";

export default function ApproverNavbar({ onWidthChange }) {
  ...
  const userRole = session?.role_label || "Approver";
            ...
            <button key={path}
              ...
              {(!collapsed || isMobile) && (
                <span style={{
                  ...
                  color: active ? ACTIVE_COLOR : "#374151",
                  whiteSpace: "nowrap",
                }}>
                  {name}
                </span>
