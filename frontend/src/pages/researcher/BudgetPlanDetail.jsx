...
const CAT_COLORS = {
  Personnel: "#1f7a1f", Equipment: "#7c3aed", Travel: "#d97706",
  Supplies: "#2563eb", Services: "#dc2626", Other: "#059669",
};
...
export default function BudgetPlanDetail() {
  ...
  const [form, setForm] = useState({ category: "", unit_price: "", description: "", year_no: "1", quantity: "1" });
  ...
  const years = [1, 2, 3];
  const yearTotals = years.map((y) => items.filter((i) => i.year_no === y).reduce((s, i) => s + Number(i.total_amount), 0));
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setShowModal(false);
      setForm({ category: "", unit_price: "", description: "", year_no: "1", quantity: "1" });
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
        ...
        <div className="dashboard-content">
          ...
          <div className="page-header">
            <div>
              ...
              <p style={{ margin: 0, fontSize: 13, color: "#6b7280" }}>Manage project budget and financial allocations</p>
            </div>
                ...
                return (
                  <div key={cat} style={{ marginBottom: 18 }}>
                    ...
                    <div className="bp-progress-bg">
                      <div className="bp-progress-bar" style={{ width: `${pct}%`, background: color }} />
                    </div>
                  </div>
                );
              })}
          ...
          {years.map((year) => {
            ...
            return (
              <div key={year} className="cp-section">
                ...
                <p style={{ fontSize: 13, color: "#6b7280", margin: "-8px 0 16px" }}>Total: {fmt(subtotal)}</p>
                      ...
                      {yearItems.map((item) => {
                        ...
                        return (
                          <tr key={item.id}>
                            ...
                            <td style={{ textAlign: "right", fontWeight: 500 }}>{fmt(item.total_amount)}</td>
                          </tr>
                        );
                      })}
                      <tr style={{ borderTop: "2px solid #e5e7eb" }}>
                        <td colSpan={4} style={{ fontWeight: 700, paddingTop: 14 }}>Year {year} Subtotal</td>
                        <td style={{ textAlign: "right", fontWeight: 700, paddingTop: 14 }}>{fmt(subtotal)}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
          ...
          {items.length === 0 && (
            <div className="cp-section">
              <p style={{ color: "#9ca3af", fontSize: 14 }}>No budget items yet. Add your first item!</p>
            </div>
          )}
          ...
          <div className="tm-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="cp-grid-2">
              <div className="cp-field">
                ...
                <input className="cp-input" type="text" placeholder="e.g., Personnel"
                  value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} autoFocus />
              </div>
              <div className="cp-field">
            ...
            <div className="cp-grid-2">
              <div className="cp-field">
                ...
                <input className="cp-input" type="number" placeholder="1"
            ...
            <div className="cp-field">
              ...
              <input className="cp-input" type="text" placeholder="Enter item description"
