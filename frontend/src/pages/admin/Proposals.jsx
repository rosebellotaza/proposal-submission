...
const getEvaluatorNames = (proposal) => {
  const oralPresentation =
    proposal.oral_presentation || proposal.oralPresentation || null;

  if (Array.isArray(proposal.evaluators)) {
    return proposal.evaluators.map((ev) =>
      typeof ev === "string" ? ev : ev.name
    );
  }

  if (Array.isArray(oralPresentation?.evaluators)) {
    return oralPresentation.evaluators.map((ev) => ev.name);
  }
        ...
        <div style={MO.body}>
          ...
          >
            ...
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#6b7280" }}>
              {proposal.researcher} · {proposal.department}
            </p>
            ...
            >
              <div style={MO.field}>
                <label style={MO.label}>
                  Date of Defense <span style={{ color: "#dc2626" }}>*</span>
                </label>
                    ...
                    >
                      ...
                      >
                        ...
                        <div>
                          ...
                          >
                            {ev.expertise || "No expertise listed"} ·{" "}
                            {ev.position || "Evaluator"}
                          </p>
                        </div>
                      </div>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
            <button style={MO.cancelBtn} onClick={onClose}>
              Cancel
            </button>
          ...
          {proposal.evaluators?.length > 0 && (
            <div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {proposal.evaluators.map((ev, index) => (
                  <span
                    key={index}
                    style={{
                      padding: "4px 12px",
...
export default function ProposalManagement() {
  ...
  const fetchProposals = async () => {
    ...
    try {
      ...
      const proposalData = Array.isArray(pRes.data)
        ? pRes.data.map((p) => {
            const oralPresentation =
              p.oral_presentation || p.oralPresentation || null;
            ...
            return {
              ...
              department:
                ...
                "—",
              budget: p.budget || p.total_budget || 0,
              submitted_date: formatDate(p.submitted_at || p.created_at),
              defense_date:
                p.defense_date ||
                oralPresentation?.presentation_date ||
                "",
              defense_time:
                p.defense_time ||
                oralPresentation?.presentation_time ||
                "",
              venue: p.venue || oralPresentation?.venue || "",
              evaluators: getEvaluatorNames(p),
              evaluator_ids:
                oralPresentation?.evaluators?.map((ev) => ev.id) || [],
              status: normalizeStatus(p.status),
            };
          })
        : [];

      const evaluatorData = Array.isArray(eRes.data) ? eRes.data : [];

      setProposals(proposalData);
      setFiltered(proposalData);
  ...
  const handleSave = async ({
    ...
    try {
      ...
      const selectedNames = evaluators
        .filter((ev) => evaluator_ids.includes(ev.id))
        .map((ev) => ev.name);
  ...
  const counts = {
    ...
    "Under Evaluation": proposals.filter(
      (p) => p.status === "Under Evaluation"
    ).length,
  };
    ...
    <>
      <style>{`
        ...
        @media(max-width:900px){
          .ppm-table-wrap { display:none; }
          .ppm-cards { display:flex; }
        }

        @media(max-width:600px){
          .ppm-stats { grid-template-columns:1fr; }
        }
      `}</style>
        ...
        >
          ...
          <div style={{ padding: "24px", flex: 1 }}>
            <div style={{ marginBottom: 24 }}>
              ...
              >
                Schedule defense dates and assign evaluators to submitted
                proposals
              </h3>
            </div>
            ...
            {success && (
              <div
                style={{
                  background: success.includes("Unable")
                    ? "#fef2f2"
                    : "#dcfce7",
                  color: success.includes("Unable") ? "#dc2626" : "#15803d",
                  padding: "12px 16px",
                  borderRadius: 10,
                  marginBottom: 20,
                  fontSize: 14,
            ...
            <div className="ppm-stats">
              {[
                ...
                <div
                  key={label}
                  onClick={() =>
                    setStatusFilter(statusFilter === label ? "All" : label)
                  }
                  style={{
                    background: statusFilter === label ? bg : "#fff",
                    border: `1px solid ${
                      statusFilter === label ? border : "#e5e7eb"
                    }`,
                    borderRadius: 12,
                    padding: "16px 20px",
                    display: "flex",
